...
char **argv; 
{
    ...
    for(i=2; i<=TestSet.m; i++)
      TestSet.X[i]=TestSet.X[i-1]+TestSet.d; // <=> X[i] <- X[i-1][1] il sectionne son espace memoire en matrice
    ...
    W[1]=(double *)malloc((size_t)(((TestSet.d+1)*T)*sizeof(double))); //il sectionne son espace memoire en matrice
    if(!W[1]){
        printf("AdaBoost-test: Allocation problem\n");
        exit(0);
    }
