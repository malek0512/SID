...
Adaboost_prof <- function(T = "fake_parameter", eta = "fake_parameter", max_iterations = 100, X, Y) {
...
evaluate_model <- function (df = NULL, model = perceptron, model_test = model_test) {
  ...
  for(t in 1:20)
  {
    ...
    for(eta in etas)
    {
      if (identical(model, Adaboost_prof) | identical(model, Logistique_prof)) 
      {
        errors = etas
        break;
      }
  ...
  return (list(mean_error_estimation = mean_error_estimation)) #, df = result2
}
...
models = c("perceptron", "adaline", "Logistique_prof", "Adaboost_prof")
#models = c("Adaboost_prof", "Logistique_prof")
#models = c("Logistique_prof")
...
for(model in models) 
{
  ...
  for(database in uci)
  {
    ...
    data = rbind(data, data.frame(model, database, error = res$mean_error_estimation))
  }  
}
...
Nous remarquons que Adaline est plus performant que perceptron sur l'ensemble des bases UCI
       model      database      error
1 perceptron breast.cancer 0.03157895
2 perceptron    ionosphere 0.11363636
3 perceptron      spambase 0.30525630
4    adaline breast.cancer 0.01140351
5    adaline    ionosphere 0.04034091
6    adaline      spambase 0.18731538

data
       model      database      error
1 perceptron breast.cancer 0.02631579
2 perceptron    ionosphere 0.10625000
3 perceptron      spambase 0.35838401
4    adaline breast.cancer 0.01754386
5    adaline    ionosphere 0.03465909
6    adaline      spambase 0.23788010
            model      database      error
1 Logistique_prof breast.cancer 0.00994152
2 Logistique_prof    ionosphere 0.06818182
3 Logistique_prof      spambase 0.14952215

          model      database       error
1 Adaboost_prof breast.cancer 0.005847953
2 Adaboost_prof    ionosphere 0.022727273
3 Adaboost_prof      spambase 0.167680278
...
En effet comme ceci à pu être étudié en cours adaline propose une meilleurs minimisation du risque empirique